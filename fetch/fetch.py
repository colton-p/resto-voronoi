...
class Tims:
    ...
    def clean(data):
        ...
        return [
            (id, node['name'],
             node['physicalAddress']['stateProvinceShort'],
             node['physicalAddress']['country'],
...
class Mcds:
    ...
    def clean(data):
        return [
            (id,
             node['properties'].get('name') or node['properties'].get(
                 'customAddress') or node['properties']['longDescription'],
             node['properties']['subDivision'],
             id.split('-')[1],
...
class Sbux:
    ...
    def clean(data):
        return [
            (id, node['name'],
             node['address']['countrySubdivisionCode'],
...
def fetch(cls, lat, lon, radius=100):
    ...
    return len(data), {x[0] for x in short}
...
def main():
    ...
    points = json.load(open('extra_starbucks.json'))
    N = len(points)

    ALL = {x[0] for x in json.load(open('data/sbux.json'))}
    print(len(ALL))

    for (ix, (lat, lon)) in enumerate(points):
        (c, ids) = fetch(Sbux, float(lat), float(lon), radius=50)

        new = len(ids - ALL)
        ALL |= ids
        print(f'{ix}/{N}', (lat, lon), f'new={new}', c)
